	 ...
	 (let val {qualid,info=S} = lookupEnv ME i
	  in
		  ...
		  if i = #uName(!currentSig) then
		      ...
		      let val cu = findAndMentionSig loc i (* cvr: REVIEW maybe findAndMention? *)
		      in
			  ([],{qualid = {qual = i,id = []},
...
(* cvr: TODO reinstate pending on conformance 
...
and checkNoRebindingsStr loc (modids,funids,tycons,vids) S msg = 
    case S of 
       STRstr (ME,FE,TE,VE) =>
        (checkNoRebindingsModEnv loc modids ME msg,
	 checkNoRebindingsFunEnv loc funids ME msg,
         checkNoRebindingsTyEnv loc tycons TE msg,
         checkNoRebindingsVarEnv loc vids VE msg)
    |  SEQstr (S,S') => 
            checkNoRebindingsStr loc (checkNoRebindingsStr loc (modids,funids,tycons,vids) S msg) S' msg
in
...
(* cvr: TODO The following, commented line should be enough to ensure that
    datbinds are insensitive to constructor ordering, as required by
    the definition. However, it breaks the current implementation of
    some runtime primitives that rely on constructors being assigned
    tags according to their order of appearance, not lexicographic
    order. Until the assumption is removed from the run-time, we'll
    just live with the current implementation restriction.  val
...
and elabDec (ME:ModEnv) (FE:FunEnv) (GE:SigEnv) (UE:UEnv) (VE:VarEnv)
  ...
  | ABSTYPEdec(dbs, tbs_opt, dec2) =>
      ...
      in
        ...
        absTE dbsTE; 
        setEquality tbsTE;
        (VE2, plusEnv(plusEnv dbsTE2 tbsTE) TE2)
        *)
        (* cvr: by *)
...
(* cvr: TODO need to generalise this *)
  | OPENdec longmodidinfos =>
(*
        EXISTS([],
	  foldL (fn ii => fn (ME,FE,GE,VE,TE) =>
		 let val {qualid = {id = [id],...},...} = ii
		     val CSig = findSig loc id 
		 in
		     (bindTopInEnv ME (modEnvOfSig CSig),
		      bindTopInEnv FE (funEnvOfSig CSig),
		      bindTopInEnv GE (sigEnvOfSig CSig),
		      bindTopInEnv VE (varEnvOfSig CSig),
		      bindTopInEnv TE (tyEnvOfSig CSig))
		 end)
  ...
  *)
  EXISTS([],
	 foldL (fn (longmodid,envoptref) => fn (ME',FE',GE',VE',TE') =>
		...
		in  
		    idKind := { qualid=csqualid, info=STRik }; (* cvr: TODO VARik should be STRik *)
		    idFields := fields;
		    envoptref := SOME Env;
		    (plusEnv ME' ME'',
		     plusEnv FE' FE'',
...
(* cvr: TODO revise above and remove below 
      let val VE' =
            foldL (fn ii => fn acc =>
                     let val {qualid = {id = [id],...},...} = ii in
          ...
          val TE' =
            foldL (fn ii => fn acc =>
                   ...
                   NILenv ids
      in EXISTS([(*cvr:TODO*)],(NILenv,NILenv,NILenv,VE', TE')) end
*)
  ...
  | STRUCTUREdec mbs => 
      ...
      (* cvr: TODO insert fo check on mbs and rebinding check *)
  | FUNCTORdec fbs => 
      let val EXISTS(T,FE') = elabFunBindList ME FE GE UE VE TE  fbs 
      in    EXISTS(T,(NILenv,FE',NILenv,NILenv, NILenv))
      end
      (* cvr: TODO insert fo check on fbs and rebinding check *)
  | SIGNATUREdec sbs => 
...
(*
    VALspec (tyvarseq,vds) => 
      (* cvr: TODO treat bound variables *)
      let val VE' = 
	      foldL_map (fn(locid, sc) => fn acc => 
  		           bindOnceInEnv acc locid sc
			     "the same value identifier is specified twice\ 
			      \ in a value specification")
			 (elabValDesc ME FE GE UE VE TE)
                         NILenv vds
...
*)
	   ...
	   val VE' = 
	      foldL_map (fn(locid, info) => fn acc => 
			 ...
			 (elabValDesc ME FE GE UE' VE TE tvs)
                         NILenv vds
       in decrBindingLevel();
 	  LAMBDA([],STRstr (NILenv, NILenv, NILenv, VE'))
       end
 | PRIM_VALspec (tyvarseq,pbs) => 
  ...
  | TYPEDESCspec(equ, tds) =>
      ...
      in
         LAMBDA(T',STRstr(NILenv, NILenv, TE',NILenv)) (* cvr: TODO handle T'*)
      end
  | TYPEspec tbs =>
      let val _ = app checkTypBind tbs;
           val tbsTE = elabTypBindList ME FE GE UE VE TE tbs in
        setEquality tbsTE;
        LAMBDA([],STRstr(NILenv, NILenv, tbsTE, NILenv))
      end
  | DATATYPEspec(dbs, tbs_opt) =>
      ...
      in
        ...
        setEquality tbsTE;
        decrBindingLevel();
        LAMBDA(T,STRstr(NILenv,  NILenv, plusEnv dbsTE' tbsTE,VE'))
      end
  | DATATYPErepspec rep => 
    let val (VE,TE) = elabDatatypeReplication ME FE GE UE VE TE (loc,rep)
    in
       LAMBDA([],STRstr(NILenv, NILenv, TE, VE))
    end    
  | EXCEPTIONspec eds =>
      (if U_map unguardedExDesc eds <> [] then
         errorMsg loc "Type variables in an exception description"
       else (); (*cvr: should be removed? *)
       LAMBDA([],STRstr(NILenv, NILenv, NILenv,elabExDescList ME FE GE [] VE TE onTop eds)))
  | STRUCTUREspec mds =>
      let val LAMBDA(T,ME') = elabModDescList ME FE GE UE VE TE mds
      in LAMBDA(T,STRstr (ME', NILenv, NILenv, NILenv)) end     
  | FUNCTORspec mds =>
      let val LAMBDA(T,FE') = elabFunDescList ME FE GE UE VE TE mds
      in LAMBDA(T,STRstr (NILenv, FE', NILenv, NILenv)) end       
  | LOCALspec (spec1, spec2) =>
      let val (ME',FE',GE',VE', TE') = elabLocalSpec ME FE GE UE VE TE (* cvr: could be true too *) spec1
      in
...
(*
  | OPENspec ids =>
      ...
      in (VE', TE') end
*)
  | EMPTYspec => LAMBDA([],STRstr(NILenv,  NILenv, NILenv, NILenv))
  | INCLUDEspec sigexp =>
      let val LAMBDAsig(T,M) = elabSigExp ME FE GE UE VE TE sigexp
  ...
  | SHARINGTYPEspec (spec,longtyconlist) => 
      let val LAMBDA(T,S) = elabSpec ME FE GE UE VE TE onTop spec 
	      ...
	      let val S' = STRstr(MEofStr S,FEofStr S,TEofStr S,VEofStr S) 
  ...
  | SEQspec (spec1, spec2) =>
      let val LAMBDA(T',S)  = elabSpec ME FE GE UE VE TE onTop spec1
          ...
          val _ = refreshTyNameSet PARAMETERts T';
          val LAMBDA(T'',S') =
            elabSpec (plusEnv ME (MEofStr S)) (plusEnv FE (FEofStr S)) GE UE (plusEnv VE (VEofStr S)) (plusEnv TE (TEofStr S)) onTop spec2
      in  decrBindingLevel();
