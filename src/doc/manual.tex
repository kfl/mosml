% manual.tex v. 2.00.0 Copyright (C) Peter Sestoft 1994, 2000-06-15
%
...
Version 2.00 of June 2000 --- draft 2000-06-15\\[0.5cm]
...
\subsection{The meaning of toplevel-mode compilation}
...
\noindent should be equivalent to the following:
...
compilation unit (file).  Projects can be organized hierarchically.

The current \texttt{mosmlpm} implementation has the following limitations: (1)
source files must be have extension .sml --- it cannot handle files
with extension .sig; (2) it does not resolve filename conflicts in
...
Each project is described by a \emph{project file}.  A project file
has file extension \texttt{.pm} and must follow this grammar:
...
source files that included inside \texttt{local}.
...
from Section~\ref{sec-example-units}.  To compile these files with PM
we first have to rename all \textit{name}\texttt{.sig} files to
\textit{name}\texttt{-sig.sml} becase PM cannot handle \texttt{.sig}
files (yet).
