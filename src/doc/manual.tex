% manual.tex v. 2.00.0 Copyright (C) Peter Sestoft 1994, 2000-06-22
%
...
% This is fine for interactive color output devices (i.e. computer displays):
%\usepackage[pdfpagemode=None,colorlinks,urlcolor=blue]{hyperref} 

% This is for passive black and white output devices (i.e. printers):
% -- the colors are awful when printed in black and white 
\usepackage[pdfpagemode=None,colorlinks=false]{hyperref}
...
\subsection{An example program consisting of four mixed-mode units}
\label{sec-example-units}
...
{\tt Reduce} and two toplevel mode units {\tt Evaluate} and {\tt Test}.  This example is in {\tt
  mosml/examples/units}.

File {\tt Expr.sml} below, is a structure mode implementation that 
contains structure {\tt Expr} which defines
a datatype {\tt expr} for representing expressions and a function {\tt
 show} to display them.  It has no signature constraint and therefore
exports both the datatype and the function:
...
\noindent File {\tt Reduce.sig} below is a structure mode interface 
...
\noindent File {\tt Reduce.sml} below is a structure mode implementation 
  that declares the structure {\tt  Reduce}, 
  with the explicit signature shown above, and therefore
  exports only the function {\tt reduce} specified in that signature:
...
\noindent File {\tt Evaluate.sig}  below is a toplevel mode interface that contains the specification of
  a functor {\tt  Eval}, which maps any structure {\tt R} matching the signature
  {\tt Reduce}  to a structure with a function {\tt eval} for evaluating
  expressions, and a function {\tt test} for testing {\tt R}.  Note the use of `{\tt open
  Expr}' to make the type {\tt expr} refer to {\tt Expr.expr}:
...
\noindent File {\tt Evaluate.sml} below is a toplevel mode implementation that contains the actual functor {\tt
  Eval}, which mentions the unit {\tt Expr} as well as well as the unit interface {\tt Reduce}:
...
\noindent File {\tt Test.sml} (a toplevel unit implementation)
  which mentions the unit {\tt Expr}, the unit {\tt Reduce}, and the functor {\tt Eval}
  defined in the unit {\tt Evaluate},
  applies the functor to {\tt Reduce} and carries out two tests.
...
\begin{program}
mosmlc -structure Expr.sml Reduce.sig Reduce.sml -toplevel Evaluate.sig Evaluate.sml Test.sml
\end{program}

which is equivalent to the individual compilations (and does not avoid unnecassary recompilations):
\begin{program}
...
\subsection{Recompilation management using {\tt make} and {\tt mosmldep}}
...
consisting of the four units {\tt Expr},  {\tt Reduce}, {\tt Evaluate} and {\tt Test}.  Assume
their source files {\tt *.sig} and {\tt *.sml} reside in a particular directory (or
look in the \texttt{mosml/examples/units}.
Copy the Makefile stub from \texttt{mosml/tools/Makefile.stub} to that
directory, and change to that directory.
...
\item\label{zero} Edit the Makefile so that the names of the units
  ...
  with `{\tt UNITS:}', with each unit preceding by an option indicatings its mode
  (see the example makefile in \texttt{mosml/examples/manual/Makefile}):

  \begin{program}
  UNITS=-structure Expr Reduce -toplevel Evaluate Test
  \end{program}

\item\label{one} Edit the Makefile so that the names of the bytecode files
  {\tt Expr.uo}, {\tt Evaluate.uo} and {\tt Reduce.uo} and {\tt Test.uo} appear on the line beginning
  with `{\tt all:}':
...
The unit dependencies are computed by the ML program
...
When you explictly supply the units,
\begin{program}
UNITS= -structure Expr Evaluate -toplevel Reduce Test
\end{program}
(together with the indication of their compilation
mode), {\tt mosmldep} merely 
constructs a dependency file on 
the assumption that units to the right depend on all preceding units, taking
into account the presence of explicit interfaces {\\tt .sig} files.
Unfortunately, this means that {\tt mosmldep} will generally create an inefficient Makefile,
containing more dependencies than are actually in your source files.
However, it still saves some recompilation (which is cheap in Moscow ML anyway) and
is more convenient than hand-crafting the Makefile.
We hope to improve  on this situation in the future, but computing dependencies
for full Standard ML programs is known to be difficult.

If you omit step (\ref{zero}) and leave {\tt \$(UNITS)} undefined, ie.:
\begin{program}
UNITS=
\end{program}
then {\tt mosmldep} will accurately {\emph infer} the dependencies between
all the {\tt .sig} and {\tt .sml} source files in the directory by inspecting
their contents.
Unfortunately, this process only works with programs containing only
structure mode units, that themselves contain only flat structures, 
but no functors or nested structures. This is the behaviour of {\tt mosmldep}
in releases of Moscow ML prior to 2.00, and is provided for backwards compatibility.
...
\item relaxations of miscellaneous Standard ML restrictions;
...
the extensions.
The directory {\tt mosml/examples/modules} contains simple examples of
these extensions (see the {\tt README file}).
...
\noindent 
A functor may be declared to take another functor as an argument,
and there is a syntax for anonymous functors:
...
\noindent
Wherever a functor of a certain type is expected, a
functor that has more a general type, ie.\  that is more polymorphic, 
expects a less general argument or produces a more general result, 
...
The files {\tt mosml/examples/modules/poly.sml,bootstrap.sml}
contain examples that use higher-order functors.
...
\noindent
(A conditional expression requires both branches to have equivalent types,
 so \texttt{res} simply tests that
 the type \texttt{SG1.t} is equivalent to \texttt{SG2.t}.)

\noindent 
If functors had an applicative, not generative,
...
\noindent
More generally, if the type in the 
functor's body depends on a datatype or opaque type of 
the functor's formal argument, 
then the types returned by separate applications of the functor
will be equivalent, provided it is applied to equivalent
type arguments:
\begin{program}
...
Moscow ML supports type projections (the dot notation for types)
with local structure bindings:
this makes it possible to refer directly to a type returned by 
an applicative functor within another type expression, and
is useful for expressing sharing constraints:
...
The local binding has no run-time effect and is only evaluated at 
compile-time, for its type information.

The file {\tt mosml/examples/modules/collect.sml,bootstrap.sml}
contains examples applicative functors.
...
to the distinction between generative and applicative functor expressions,
functor signatures may be \emph{opaque} or \emph{transparent}.
Whether
a functor signature is opaque or transparent affects the interpretation
of any datatype or opaque type specifications in its range:

The \emph{opaque} signature 
\begin{program}
signature GO = functor(X:sigtype t val x:t end)->sig type u val y:u end   (* opaque *)
\end{program}

\noindent
specifies the type of those functors that, when applied to
an actual argument matching the domain signature
\texttt{sig type t val x : t end},
return a result 
matching the range signature
\texttt{sig type u val y:u end}, for some unknown implementation of the type 
\texttt{u} (possibly depending on \texttt{X.t}).


An \emph{transparent} version \texttt{GT} of the above functor signature is
written the same way, except that the formal functor argument
\texttt{X:sig type t val x: t end} is \emph{not} enclosed in parentheses:
\begin{program}
...
map structures matching the domain 
\texttt{sig type t val x:t  end} to structures
matching the range \texttt{sig type u val y:u end}.
...
\noindent
\texttt{F1} and \texttt{F2} are first-order functors: 
\texttt{F1} returns a renamed version of its argument;
and \texttt{F2} just ignores it argument and returns
the same structure regardless.
The higher-order functors \texttt{HO} and \texttt{HT}
merely apply the supplied \texttt{F}, to the same argument, but assume,
respectively, an opaque and transparent signature for \texttt{F}.

Since \texttt{HO} uses the opaque signature \texttt{GO}, its formal argument
\texttt{F}\ is
assumed to return some new unknown type \texttt{u} whenever it is applied,
so that the two applications of \texttt{HO} return new abstract types
\texttt{RO1.u} and \texttt{RO2.u}.
...
\texttt{HT}, on the other hand, uses the transparent signature \texttt{GT}.
This ensures that,
no matter what the actual dependency of type \texttt{u} on type \texttt{t},
\texttt{HT} may be applied to any functor \texttt{F} whose domain
and range match \texttt{GT}: with the actual
definition of \texttt{u} reflected in result. % the range of \texttt{HT}.
In particular,
the two applications of \texttt{HT} return the same
definitions for type \texttt{u} as substituting \texttt{F1} 
and \texttt{F2} directly into the body of \texttt{HT}: 
ie. the
types \texttt{RT1.u} and \texttt{RT2.u} are respectively
equivalent to \texttt{string} and \texttt{int}:
...
a generative specification,
while \texttt{HT}'s formal argument has an
...
% end
% type bundle = [ BUNDLE ]	                        (* package type *)
% fun g (bpack : bundle) =
...
val natlist = [safeNat,fastNat] : [ NAT ] list;             (* etc. *)

\end{program}

A functor may be packed using the similar Core expression
\texttt{ [ functor {\it modexp} as {\it sigexp} ]} and
unpacked using  the functor binding 
\texttt{ functor {\it funid} as {\it sigexp} = {\it exp}}.

Package types are compared by structure, not name, so
the package type:
\begin{program}
...
\end{program}
\noindent 
because the signatures are equivalent (every structure that
matches one also matches the other).

For type soundness reasons, a package may not be unpacked in the body of a functor (although it may be unpacked within a Core expression occuring in 
that  body).
...
The files {\tt mosml/examples/modules/sieve.sml,array.sml,choice.sml,matrix.sml}
...
modules and disallows, for instance, two structures \texttt{Even} and \texttt{Odd}
that both depend on each other.  

In the Moscow ML Modules language, a structure may be defined recursively:

\begin{program}
structure S = rec (X:sig structure Odd : sig val test : int -> bool end end)
              struct sructure Even = struct fun test 0 = true 
...
\noindent
Attempting to evaluate the foward reference 
of a recursive structure before its body has been
been fully evaluated raises the Moscow ML exception \texttt{Bind}:
...
\noindent
On their own, recursive structure can not be used to declared 
mutually recursive datatypes that span module boundaries. 
For this purpose, Moscow ML supports recursive signatures:
...
\noindent
Here, \texttt{X}\ is a forward declaration of a structure
implementing the body of the signature that
allows the specification of \texttt{Even.t} to refer to
the type \texttt{X.Odd.t} before it has been fully specified.
In a recursive signature, the body of the signature must 
match the forward declaration and
specify an implementation for any opaque types or datatypes
declared  within the forward specification.

\noindent
Once this recursive signature has been defined, it can then 
be used to implement the recursive structure \texttt{T}
with datatypes that span module boundaries.
...
The files {\tt mosml/examples/modules/recursion.sml,bootstrap.sml}
...
In Standard ML, functors and signatures may only be declared
at top-level and structures may only be declared at top-level and
within structures. None of these may be declared
within Core let expressions.
Moscow ML removes these restrictions so that functors, signatures
and structures may be declared anywhere, which is particularly useful when
programming with first-class modules.

In Standard ML, every parameterised type definition, and evey type scheme
occuring within a signature,
must be closed and not mention any type variables that are not explicitly
listed as parameters. Moscow ML does not impose this restriction, and
allows free type variables, provided they are bound in an enclosing scope.
Again, this is can be useful when programming with first-class modules.
