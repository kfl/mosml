% manual.tex v. 2.00.0 Copyright (C) Peter Sestoft 1994, 2000-06-26
%
...
Version 2.00 of June 2000 --- draft 2000-06-26\\[0.5cm]
...
\noindent 
File {\tt Evaluate.sig} below is a toplevel mode interface that
...
\subsection{Mixed-mode recompilation management using mosmldep and make}
\label{sec-mixed-recompilation}
...
reside in a particular directory (or look in the
...
\noindent When you explictly list the units (together with the indication of
...
\noindent {\tt mosmldep} merely constructs a dependency file on the
assumption that units to the right depend on all preceding units,
taking into account the presence of explicit interfaces {\tt .sig}
files.  Unfortunately, this means that {\tt mosmldep} will generally
create an inefficient Makefile, containing more dependencies than are
actually in your source files.  However, it still saves some
...
directory by inspecting their contents.  Unfortunately, this process
only works with programs containing only structure mode units, that
themselves contain only flat structures, but no functors or nested
structures.  This is the behaviour of {\tt mosmldep} in releases of
...
the extensions.  For more examples, see the directory {\tt
...
equivalent types, so the last valbind above is well-typed only if the
...
will be equivalent, provided it is applied to equivalent type
arguments:
\begin{program}
...
argument matching the argument signature \texttt{sig type t val x :\ t
  end}, return a result matching the body signature \texttt{sig type u
  val y:u end}, for some unknown implementation of the type \texttt{u}
...
a generative specification, while \texttt{HT}'s formal argument has an
...
\noindent
Attempting to evaluate the forward reference of a recursive structure
...
Moscow ML supports recursive signatures:
