% manual.tex v. 2.00.0 Copyright (C) Peter Sestoft 1994, 2000-06-16
%
...
Version 2.00 of June 2000 --- draft 2000-06-16\\[0.5cm]
...
\item The Moscow ML Modules language extends the Standard ML Modules
  ...
  \item recursive modules: signatures, structures, and functors may be
...
provides much information about the declaration, once you know how to
read it.  

When reporting the type of a value binding, the generalized variables
are listed between the \texttt{val} keyword and the identifier(s)
...
\noindent
The notation \texttt{=u'} means that the type name \texttt{u'} admits
equality.  Since the \texttt{datatype} declaration is generative,
repeated declaration will produce distinct internal new type names
\texttt{=u''}, \texttt{=u'''}, etc.  If the datatype is polymorphic,
the type name will be a type function, taking as parameter a type
variable \texttt{'a}:
...
has three components (\texttt{t}, \texttt{A}, and \texttt{x}):
...
\noindent When a standard generative functor \texttt{G}
whose body contains a \texttt{datatype} declaration, the response
describes which new (internal) type names will be generated at every
application of the functor.  The notation \verb+{}->...+ means that
the functor argument signature is empty, and the notation
\verb+?=t'.{...}+ says that there is some type name \verb+t'+,
admitting equality, such that the functor body has the signature
\verb+{...}+:
...
Section~\ref{sec-applicative-functors}) then no new type names are
generated.
...
\item \textbf{Structure mode:} File \texttt{unitid.sml} declares a
  ...
  constraint for that structure.  
  
  A structure-mode unit has a restricted syntax: it can declare only a
  single non-nested structure, and therefore admits simple
  recompilation management using \texttt{mosmldep} and makefiles; see
...
structure-mode units, and the project manager \texttt{mosmlpm} (see
Section~\ref{sec-project-manager}) will compile all units in toplevel
mode.
...
A structure-mode compilation unit consists of a {\em unit
  implementation}, in file unitid.sml, and an optional {\em unit
  interface}, in a file called unitid.sig\@.  The file unitid.sml must
contain a single Moscow ML structure declaration.

The unit interface specifies the contents of the unit.  The unit
implementation declares the contents of the unit.  When the unit
interface is present, it is called the {\em explicit signature\/} to
distinguish it from the signature inferred when elaborating the unit
implementation.  When present, the explicit signature must be matched
by the implementation, and only those identifiers specified in the
signature are visible outside the unit.  If no signature is given, all
identifiers visible at the end of the unit implementation are visible
outside the unit.

For a structure-mode unit it is immaterial whether a compilation
unit's interface and implementation is compiled and directly executed
...
then compilation inside the interactive toplevel system:
...
TODO: the meaning of .sig and .sml files in this context.

For a toplevel-mode unit it is immaterial whether a compilation unit
is compiled and directly executed in the interactive toplevel system:
...
filename conflicts in different (sub)projects.
...
\noindent This file can then be compiled and linked ????
with the command:

\begin{program}
mosmlpm -c evaluator.pm
\end{program}
...
structure, or functor may be defined recursively:
...
the body of generative functor \texttt{F} declares a datatype
\texttt{t}, then two applications of \texttt{F} will create two
structures with distinct types \texttt{t}.  If instead \texttt{F} were
an applicative functor, then the two types would be identical.  Moscow
ML Modules allows the declaration of applicative functors\footnote{As
  in Objective Caml.} as well as generative functors.  Applicative
functors are declared the same way as generative functors, except that
the formal functor argument is not enclosed in parentheses:
...
\item Emden Gansner and John Reppy (editors): Standard ML Basis
  ...
  \url{http://www.dina.kvl.dk/~sestoft/sml/sml-std-basis.html}
  (mirror site)
