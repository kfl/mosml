...
implements parts of the SML Basis Library.  
\delete{Section~\ref{sec-libindex}
...
{\bf Acknowledgement:} The present document owes a lot to the {\em
  Definition\/}. 
  \insert{Thanks to Don Sannella at LFCS,
	  Division of Informatics, University of Edinburgh for 
	  funding C. Russo under
...
In the grammar, a module identifier {\it longmodid} may stand for either a structure identifier
{\it longstrid} or a functor identifier {\it longfunid}.
...
% The unit's implementation (and interface, if any) must have been compiled in \emph{structure} mode.
 Any occurrence of a structure identifier {\it strid} that is not bound in the current context refers to the unit implementation  {\it unitid.uo} 
of the same name (ie. {\it unitid} = {\it strid}).
 At compile time, the unit's compiled interface {\it unitid.ui} must exist and have been  compiled in \emph{structure} mode.
 At link time, the unit's compiled implementation {\it unitid.uo} must exist and have been  compiled in \emph{structure} mode.
}
\revise{\item 
% An occurrence of a signature identifier
...
% The interface of the unit must exist and have been compiled in \emph{structure} mode.
 Any occurrence of a signature identifier {\it sigid} that is not bound in the current context refers to the compiled unit interface {\it unitid.ui} of the same name (ie. {\it unitid} = {\it sigid}).
 The file {\it unitid.ui} must have been compiled
 in \emph{structure} mode  from
 an explicit interface {\it unitid.sig}.
}
\end{itemize}
...
& & \insert{{\tt structure {\it strbind}}}
& \insert{structure declaration} & \x[\ref{strdec}]\\
& & \insert{{\tt functor {\it funbind}}}
& \insert{functor declaration} & \x[\ref{topdec}]\\
& & \insert{{\tt signature {\it sigbind}}}
& \insert{signature declaration} & \x[\ref{topdec}]\\\\
& & & empty declaration\\
...
& & \insert{{\tt {\it longtycon\/} where {\it strid\/} = {\it modexp\/}}}& \insert{type constructor projection} &\x\\[2ex]

{\it ty\/} & ::= & {\it tyvar\/} & type variable\\
& & {\tt \{ \la\ {\it tyrow\/} \ra\ \}} & record type expression\\
& & {\tt {\it tyseq\/} \replace{{\it longtycon\/}}{{\it tyconpath\/}}} & type construction & %\x
\\
...
\item\label{strdec} \insert{
  For Standard ML compliance (enforced by  Moscow ML's \emph{orthodox} mode),
  the structure declaration   {\tt structure {\it strbind\/}}
...
\item\label{topdec} \insert{
  For Standard ML compliance (enforced by  Moscow ML's \emph{orthodox} mode),
  the functor declaration   {\tt functor {\it funbind\/}} and
  signature declaration   {\tt signature {\it sigbind\/}}
  may only occur at the top level of a program, but not within the declarations of a structure or Core 
  {\tt let}-expression.}
...
{\it funbind\/} & ::= & {\tt {\it funid\/} {\it arg}\et\ $\cdots$\ {\it arg}\n\ \la\ {\it con\/} \ra\ = {\it modexp}} & \\
                &     & \quad {\tt \la\ and {\it funbind} \ra} & functor binding, $n \geq 1$ & \x[\ref{funbind}]\\
...
\item\label{atmodexp} \insert{
  For Standard ML compliance (enforced by Moscow ML's \emph{orthodox} mode),
  the phrase must be an application of a functor identifier to a single argument of the form:}

  \insert{\begin{tabular}{l}
  {\tt {\it funid\/} ( {\it atmodexp\/} )}\\
  {\tt {\it funid\/} ( {\it dec\/} )}
  \end{tabular}}

\item\label{funbind} \insert{
  For Standard ML compliance (enforced by Moscow ML's \emph{orthodox} mode),
  the phrase must define a one-argument, generative functor and be of the form:}
...
\item\label{valspec} \insert{
  For Standard ML compliance (enforced by Moscow ML's \emph{orthodox} mode),
  in a value specification {\tt val} {\it tyvarseq\/} {\it valdesc\/},
  {\it tyvarseq\/} must be empty, and the specification of the form:}
...
\delete{The Moscow ML module language is a simplified form of the full
Standard ML Modules language.}

\delete{The equivalent of a Standard ML structure is a unit.  A unit U
...
\begin{insertion}
 Moscow ML supports the separate compilation of named program fragments called \emph{units}.
 A unit {\tt unitid} consists of an optional \emph{unit interface} in file {\tt unitid.sig} \delete{(corresponding to an SML
 signature)} and a \emph{unit implementation}  in file {\tt unitid.sml} \delete{(corresponding to an SML
 structure)}.  
 Each unit can be compiled in one of two \emph{modes}: \emph{structure} mode and \emph{toplevel} mode.
 A unit's implementation and interface files must be compiled in the \emph{same} mode. 

 In the batch compiler {\tt mosmlc}, a unit's compilation mode is specified by
 preceding it with the command-line argument {\tt -structure} (the default) or {\tt -toplevel}.
 In the interactive system {\tt mosml}, the compilation mode of a unit is determined
 by the function with which it is compiled: {\tt compile} and {\tt compileStructure}
 compile in \emph{structure} mode; {\tt compileToplevel} compiles in \emph{toplevel} mode.

 Note that the intended mode of a unit is not determined by file name extension or by file
 content: the mode  must be explicitly indicated to the batch compiler and interactive system.

 The syntax and semantics of a unit's interface and implementation files depends on the mode 
 and is described in the following sections.

%  The Modules equivalent of a Moscow ML \emph{unit} depends on the mode in the unit was compiled.
%  In either case, a unit
%  consists of a \emph{unit interface} in file {\tt unitid.sig} \delete{(corresponding to an SML
%  signature)} and a \emph{unit implementation}  in file {\tt unitid.sml} \delete{(corresponding to an SML
%  structure)}.  The unit interface may omitted.
...
% In \emph{structure} mode, the unit interface in file U.sig is a single Moscow ML signature declaration, and the unit implementation in file U.sml is a single Moscow ML structure binding. 
...
\begin{tabular}{lllll}
{\it unitint\/} & ::=   & {\it spec\/} & specification & \x
\end{tabular}
...
\insert{substring}   & \insert{\tt string * int * int -> string} & get substring
$(s, first, len)$ & \insert{\tt Subscript}\\
