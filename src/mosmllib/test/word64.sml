(* test/word64.sml -- some test cases for Word, appropriate for a two's
   complement machine.

   Based on word.sml by PS
   2021-07-05 KFL
 *)
...
val test9b = checkrange (valOf Int.precision, valOf Int.precision + 40)
                        (fn k => 0 = w2i (<< (i2w 1, i2w k)));
val test9c =
    (w2i (<< (i2w 1, i2w(valOf Int.precision - 1))); "WRONG")
...
val test9g = checkrange (valOf Int.precision, valOf Int.precision + 40)
                        (fn k => 0 = w2i (<< (i2w ~1, i2w k)));
...
val test12n = check(toIntX (i2w ~1 div i2w 4) = maxposint);
val test12o = check(w2i (i2w ~1 mod i2w 2) = 1);
val test12p = check(w2i (i2w ~1 div i2w 100) = idiv(maxposint, 25));
