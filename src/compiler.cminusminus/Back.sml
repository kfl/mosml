  ...
  let
    ...
    fun compexp sz dp exp C =
      ...
      | Lapply(body, args) =>
		...
		(* 2 is the number of stack positions used by Kpush_retaddr *)
              let val (lbl, C1) = labelCode C
              in Kpush_retaddr lbl ::
                   compExpList (sz + 2) dp args
                     (Kpush :: compexp (sz + 2 + na) dp body (Kapply na :: C1))
              end
          end
      | Lfn body =>
...
fun compileRest C =
  let val (lbl, nargs, fv, maxstk, exp) = Stack.pop stillToCompile
      ...
      val () = inienv 0 nargs
      val restart_lbl = new_name()
      val restart_fun = [Kname restart_lbl, Knewrestart lbl]

(*      val C' = compileExp env (Nolabel, 0) nargs nargs exp 
      	                  (Kreturn nargs :: discardDeadCode C)
*)
      val C' = compileExp env (Nolabel, 0) nargs nargs exp [Kreturn nargs]
  in
    compileRest (List.@(if nargs > 1
                        then [restart_fun, 
                              Kname lbl :: 
                              Knewgrab (restart_lbl, nargs - 1) :: C']
                        else [Kname lbl :: C'], C))
  end
  handle Stack.Empty => C


fun compileLambda is_pure exp =
  let val () = Stack.clear stillToCompile
      (*val () = resetLabel() *) (* In C-- labels have global scope *)
      (*val () = resetLabelNot()*)
