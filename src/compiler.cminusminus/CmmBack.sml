...
structure CmmBack =
struct
   local open CmmAST Lambda in
   
   (* "isReturn" determines if we're in tail call position. *)
   fun isReturn (Return _ :: _ )            = true
     | isReturn (Label _ :: Return _ :: _ ) = true
     | isReturn _                           = false
   ...
   *)
   ...
   fun labelCode C =
       case C of
         ...
         | _ =>
           ...
           in (lbl, Label lbl :: C) end

   (* Generate a Goto to the given list of instructions. *)
   fun makeGoto C =
       case C of
           (return as Return _) :: _            => (return, C)
         | (goto   as Goto _) :: _              => (goto, C)
         | (cut    as Cut _) :: _               => (cut, C)
         | Label _ :: (return as Return _) :: _ => (return, C)
         | Label lbl :: _                       => (Goto lbl, C)
         | _ =>
           let val lbl = new_label()
           in (Goto lbl, Label lbl :: C) end

   (* Discard all instructions up to the next label or continuation. *)
   fun discardDeadCode C =
       case C of
           []                                         => []
         | Label _ :: _                               => C
         | Call([], _, Reg"set_global", _, _, _) :: _ => C
         | Continuation _ :: _                        => C 
   ...
   fun findEnv (fv,va) sz n =
       ...
       then let fun f i [] = fatalError ("findEnv fv n: " ^ (makestring n))
            ...
            in if i < 0 then fatalError "findEnv va1" else Kaccess (sz - i) end
               handle Subscript => 
                      fatalError ("findEnv va2 n: " ^ (makestring n))
   ...
   fun findStk (fv,va) sz n =
       ...
       then fatalError ("findStk n: " ^ (makestring n))
            ...
            in if i < 0 then fatalError "findStk va1" else Kassign (sz - i) end
                handle Subscript => 
                       fatalError ("findStk va2 n: " ^ (makestring n))
   ...
   fun bindEnv (fv,va) n z =
       ...
       in
           ...
           n'
       end handle Subscript => fatalError "bindEnv"
